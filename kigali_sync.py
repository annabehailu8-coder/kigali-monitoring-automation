...
def send_telegram_alert(score, task_name, alert_image, background_image, region, time_span):
    """Sends a photo notification (Radar on Satellite) with Time Span and Red Pixels."""
    ...
    # Brightened Sentinel-2 background
    bg_vis = background_image.visualize(bands=['B4', 'B3', 'B2'], min=0, max=4000)
    
    # Radar Alerts: using .selfMask() ensures ONLY change pixels are red, not the whole box
    ...
    try:
        ...
        # 2. Prepare the Telegram payload with the new Time Span field
        caption = (
        ...
        requests.post(fallback_url, data={
            'chat_id': chat_id, 
            'text': f"🚨 Alert: {score} pixels detected in {time_span}. (Map failed).", 
            'parse_mode': 'Markdown'
        })
    ...
    if latest_img.getInfo():
        ...
        if current_id != last_id:
            ...
            if change_score > 5:
                ...
                # SENDS PHOTO ALERT: Added the time_span_str parameter here
